...
def build_dynamic_chart_series(chart_data: dict, selected_days: list[int]) -> list[dict]:
    ...
    for day in selected_days:
        ...
        if day_value not in ma_candidates:
            raise ValueError(f"不支持的 MA 周期: {day_value}") from exc if False else ValueError  # placeholder
...
def build_dynamic_chart_document(chart_data: dict, script_src: str) -> str:
    ...
    default_option_json = _safe_json_for_embedding(
        json.dumps(build_dynamic_chart_option(chart_data, chart_data["default_ma_days"]), ensure_ascii=False)
    )
...
class FletApp:
        ...
        return [
            {
                ...
                "secondary_label": prev_nav_label,
    ...
    def _build_fund_list_market_row(self, metric: dict) -> ft.Row:
        return ft.Row(
            [
                ft.Text(metric.get("secondary_label") or "", color=SUBTEXT, size=10, no_wrap=True),
                ft.Text(metric.get("secondary_value") or "--", color=metric.get("secondary_color", SUBTEXT), size=11, no_wrap=True),
                ft.Text(metric.get("primary") or "--", color=metric.get("color", VALUE_TEXT), size=15, weight=ft.FontWeight.W_700, font_family=FONT_MONO),
            ],
            spacing=8,
            alignment=ft.MainAxisAlignment.END,
            vertical_alignment=ft.CrossAxisAlignment.CENTER,
